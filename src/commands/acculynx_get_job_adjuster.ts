...
export default defineTool({
  description: "Retrieve the insurance adjuster information (name, contact details) for a specific Job by its UUID.",
  ...
  async execute({ jobId, response_format }, ctx) {
    try {
      ...
      const res = await client.getAdjusterForJob({ jobId });
      ...
      if (status === 404) {
        return formatToolResponse({ message: "No adjuster information recorded for this job." }, response_format);
      }
