...
export default defineAcculynxTool({
  description: "Retrieve the custom field values set on a specific Job by its UUID. (For the company-wide custom field definitions/IDs, use acculynx_get_custom_fields instead.)",
  ...
  async call(client, { jobId, pageSize, recordStartIndex }) {
    ...
    const res = await client.getJobCustomFields(metadata);
    return res.data;
  },
});
