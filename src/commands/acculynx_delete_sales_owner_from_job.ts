...
export default defineAcculynxTool({
  description: "Delete Sales Owner This mutates AccuLynx data and requires explicit human approval before it executes.",
  approval: always(),
  inputSchema: z.object({
    jobId: z.string().describe("The job's unique identifier"),
  }),
  async call(client, { jobId }) {
    const res = await client.deleteSalesOwnerFromJob({ jobId });
